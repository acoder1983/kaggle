...
def stl_decompose(x, model="additive", filt=None, freq=None, two_sided=True):
    _pandas_wrapper, pfreq = _maybe_get_pandas_wrapper_freq(x)
    x = np.asanyarray(x).squeeze()
    nobs = len(x)
    ...
    nsides = int(two_sided) + 1
    trend = sm.nonparametric.lowess(x,np.arange(len(x),dtype=x.dtype),frac=freq/len(x))[:,1]
